    ...
    <div class="flex items-center justify-between py-6 lg:py-10">
        ...
        <div class="hidden lg:block">
            <ul class="flex items-center">
                ...
                @foreach (explode(',', $menu) as $item)
                    <li class="group relative mr-6 mb-1">
                        <div
                            class="absolute left-0 bottom-0 z-20 h-0 w-full opacity-75 transition-all group-hover:h-2 group-hover:bg-yellow">
                        </div>
                        <a href="/{{ $item }}"
                            class="relative z-30 block px-2 font-body text-lg font-medium text-primary transition-colors group-hover:text-green dark:text-white dark:group-hover:text-secondary">{{ ucfirst($item) }}</a>
                    </li>
                @endforeach
    ...
    <div class="ml-auto w-2/3 bg-green p-4 md:w-1/3">
        ...
        <ul class="mt-8 flex flex-col">
            ...
            @foreach (explode(',', $menu) as $item)
                <li class="">
                    <a href="/{{ $item }}"
                        class="mb-3 block px-2 font-body text-lg font-medium text-white">{{ ucfirst($item) }}</a>
                </li>
            @endforeach
